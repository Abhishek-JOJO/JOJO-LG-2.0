...
const getCachedAsset = cache(async (assetId: string, sessionId?: string) => {
  try {
    ...
    console.error(`[Server Shows getCachedAsset Error] assetId "${assetId}":`, error);
    return null;
  }
});
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!assetId) {
    return buildAssetMetadata({
      title: fallbackTitle,
      description: "Watch Gujarati web series and shows online on JOJO.",
      canonicalUrl: "https://jojoapp.in",
      imageUrl: defaultOgImage,
      ogType: "video.tv_show",
      keywords: ["JOJO App", "Gujarati Web Series"],
    });
  }
  ...
  try {
    ...
    if (!asset) {
      ...
      return buildAssetMetadata({
        title: fallbackTitle,
        description: "Watch premium Gujarati web series online on JOJO App.",
        canonicalUrl: `https://jojoapp.in/shows/${fallbackSlug}/${assetId}`.replace(/\/+$/, ""),
        imageUrl: defaultOgImage,
        ogType: "video.tv_show",
        keywords: ["JOJO App", "Gujarati Web Series"],
      });
    ...
    const categoryLabel = isShow ? "Gujarati Web Series" : "Gujarati Show";

    const title = buildSeoTitle(asset.title, asset.seoTitle, categoryLabel);
    const rawDesc = asset.seoDescription || asset.description || `Watch ${asset.title || "premium content"} ${isShow ? "original episodes" : "full show"} online in HD on JOJO App.`;
    ...
    const canonicalUrl = `https://jojoapp.in/shows/${slug}/${assetId}`;
    ...
    const dynamicCanonical = assetId
      ? `https://jojoapp.in/shows/${fallbackSlug}/${assetId}`.replace(/\/+$/, "")
      : "https://jojoapp.in/shows";

    return buildAssetMetadata({
      title: fallbackTitle,
      description: `Watch ${unslugify(fallbackSlug) || "Gujarati web series"} online in HD on JOJO App.`,
...
import { buildTVSeriesSchema, buildMovieSchema, buildBreadcrumbSchema, buildVideoObjectSchema } from "@/lib/seo/schema";
...
export default async function ShowsAssetDetailPage({ params }: Props) {
  ...
  try {
    ...
    console.error("[Server ShowsAssetDetailPage Fetch Error]:", error);
  }
  ...
  try {
    ...
    if (asset) {
      ...
      breadcrumbs = buildBreadcrumbSchema([
        ...
        { name: asset.title, item: `https://jojoapp.in/shows/${slugify(asset.title)}/${asset.assetId}` },
      ]);
    }
  } catch (error) {
    console.error("[Server ShowsAssetDetailPage Metadata/Prefetch Error]:", error);
  }
