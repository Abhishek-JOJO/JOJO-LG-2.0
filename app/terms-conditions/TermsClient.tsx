...
export default function TermsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const result = await fetchPolicy(2, languageId); // policy_id 2 for Terms & Conditions
        setPolicy(result);
        setError("");
      } catch (e: any) {
        console.error(e);
        setError(e.message || t("failedTerms"));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen text-white">
        <span className="animate-pulse">{t("loadingTerms")}</span>
      </div>
    );
  }
