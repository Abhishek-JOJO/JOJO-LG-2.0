...
const getCachedAsset = cache(async (assetId: string, sessionId?: string) => {
  try {
    ...
    console.log(`[Server Kids getCachedAsset] Error resolving assetId "${assetId}":`, error);
    return null;
  }
});
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    const categoryLabel = "Kids Content";
    const title = buildSeoTitle(asset.title, asset.seoTitle, categoryLabel);
    const rawDesc = asset.seoDescription || asset.description || `Watch ${asset.title || "premium content"} for kids online in HD on JOJO App.`;
    ...
    const canonicalUrl = `https://jojoapp.in/kids/${slug}/${assetId}`;
...
export default async function KidsAssetDetailPage({ params }: Props) {
  const resolvedParams = await params;
  const slugAndIdRaw = resolvedParams.slugAndId;
  const slugAndId = Array.isArray(slugAndIdRaw) ? slugAndIdRaw : typeof slugAndIdRaw === "string" ? [slugAndIdRaw] : [];
  const assetId = slugAndId.length > 0 ? slugAndId[slugAndId.length - 1] : "";
  ...
  try {
    ...
    console.error("[Server KidsAssetDetailPage Fetch Error]:", error);
  }
  ...
  try {
    ...
    if (asset) {
      ...
      breadcrumbs = buildBreadcrumbSchema([
        ...
        { name: asset.title, item: `https://jojoapp.in/kids/${slugify(asset.title)}/${asset.assetId}` },
      ]);
    }
  } catch (error) {
    console.error("[Server KidsAssetDetailPage Metadata/Prefetch Error]:", error);
  }
